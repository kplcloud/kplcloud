...
func (c *service) TestSend(ctx context.Context, id int) error {
	...
	if err != nil {
		_ = c.logger.Log("webhook", "FindById", "err", err.Error())
		return ErrHooksGet
	}
	...
	go func() {
		if err := c.hookQueueSvc.TestHookQueue(&event.WebhooksRequest{
			...
			_ = c.logger.Log("hookQueueSvc", "HookQueue", "err", err.Error())
		}
	}()
...
func (c *service) List(ctx context.Context, name, appName, namespace string, page, limit int) (res map[string]interface{}, err error) {
	...
	if err != nil {
		_ = c.logger.Log("hook", "Count", "err", err.Error())
	...
	if err != nil {
		_ = c.logger.Log("hook", "FindOffsetLimit", "err", err.Error())
...
func (c *service) Post(ctx context.Context, req hookRequest) (err error) {
	...
	if hook, err := c.repository.Webhook().FindByName(req.Name); err == nil {
		if hook.Name != "" {
			_ = c.logger.Log("hook", "create webhook", "err", "name is exist")
			return ErrHooksNameExits
		}
	}
	eventList, err := c.repository.Event().FindByIds(req.Events)
	if err != nil {
		_ = c.logger.Log("hook", "create webhook. findEventsByIds", "err", err)
	...
	if err != nil {
		_ = c.logger.Log("hook", "create webhook database ", "err", err)
...
func (c *service) Update(ctx context.Context, req hookRequest) (err error) {
	if hook, err := c.repository.Webhook().FindByName(req.Name); err == nil {
		if hook.Name != "" && hook.ID != req.Id {
			_ = c.logger.Log("hook", "update webhook", "err", "name is exist", "id", hook.ID)
			return ErrHooksNameExits
		}
	}
	eventList, err := c.repository.Event().FindByIds(req.Events)
	if err != nil {
		_ = c.logger.Log("hook", "update webhook. findEventsByIds", "err", err)
		return ErrEventsNotFound
	}

	//关联关系更新
	if err := c.repository.Webhook().DeleteEvents(&types.Webhook{ID: req.Id}); err != nil {
		_ = c.logger.Log("hook", "delete hookEvents", "err", err.Error())
	}
	if err := c.repository.Webhook().CreateEvents(&types.Webhook{ID: req.Id}, eventList...); err != nil {
		_ = c.logger.Log("hook", "update hookEvents", "err", err.Error())
	}
	...
	if err != nil {
		_ = c.logger.Log("hook", "update webhook database ", "err", err)
		return ErrHooksCreate
	}
	return nil
}
